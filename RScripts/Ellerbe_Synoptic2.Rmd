...
Mainstem_predict <- Mainstem_predict[,c("DateTime","NO3_predict","DOC_predict")]

df_predict <- full_join(df,Mainstem_predict, by = "DateTime")



TrackMap <- qmplot(lon, lat, data = df_predict%>%drop_na(DOC_predict)
                   , zoom = 13, #color = grey,
                   maptype = "toner-background"
                   ) 

TrackMap +
  geom_point(aes(color = log10(NO3_predict)), data = df_predict %>%drop_na(DOC_predict), size = 2) +
  scale_color_gradient(low="blue", high="red") +
...
TrackMap +
  geom_point(aes(color = log10(DOC_predict)), data = df_predict%>%drop_na(DOC_predict), size = 2) +
  scale_color_gradient(low="blue", high="red") +
...
TrackMap <- qmplot(lon, lat, data = data_upstream%>%drop_na(DOC_predict)
                   , zoom = 13, #color = grey,
                   maptype = "toner-background"
                   ) 
TrackMap +
  geom_point(aes(color = NO3_predict), data = data_upstream %>%drop_na(DOC_predict), size = 2) +
  scale_color_gradient(low="blue", high="red") +
  labs(color = "Nitrate \n (mg/L)") 

TrackMap +
  geom_point(aes(color = DOC_predict), data = data_upstream %>%drop_na(DOC_predict), size = 2) +
  scale_color_gradient(low="blue", high="red") +
  labs(color = "DOC \n (mg/L)") 
